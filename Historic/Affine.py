...
def prime_denominators():
    p = set(primerange(1, MOD))                                                     # List all the primes lower than the modulus
...
def decrypt_bruteforce(enc_text):
    for key1 in prime_denominators():
        for key2 in range(MOD):
            text = ''
            for l in enc_text:
                if l in alphabet:
                    rot = (inverse(key1, MOD) * (alphabet.index(l)-key2)) % MOD   # Rotation (a^-1 * (L-b) (mod 26) for lower case letters
                    text += alphabet[rot]
                elif l in ALPHABET:
                    rot = (inverse(key1, MOD) * (ALPHABET.index(l)-key2)) % MOD   # Rotation (a^-1 * (L-b) (mod 26) for upper case letters
                    text += ALPHABET[rot]
                else:
                    text += l
...
if __name__ == '__main__':
    text = TEXT.replace(' ', '')
    key1 = secrets.choice(prime_denominators())                                   # Generate a random key, chosen among coprimes of N
    print(f'First random key, prime denominator of {MOD}: {key1}')
